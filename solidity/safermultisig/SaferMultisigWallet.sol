...
contract MultisigWallet is IAccept {
    ...
    150 - invalid stake message signature length
    */
    ...
    function _initialize(uint256[] owners, uint256 node, uint8 reqConfirms) inline private {
        uint8 ownerCount = 0;
        m_ownerKey = owners[0];
        m_nodeKey = node;
    ...
    constructor(uint256[] owners, uint256 node, uint8 reqConfirms) public {
        require(msg.pubkey() == tvm.pubkey(), 100);
        require(owners.length > 0 && owners.length <= MAX_CUSTODIAN_COUNT, 117);
        tvm.accept();
        _initialize(owners, node, reqConfirms);
    }
    ...
    function sendStake(
        ...
        uint32 maxFactor,
        uint256 adnlAddr,
        bytes signature) public view
    {
        require(msg.pubkey() == m_nodeKey, 100);
        require(signature.length == 64, 150);
        tvm.accept();

        TvmBuilder builder;
        builder.store(uint32(0x4e73744b), queryId, validatorPubKey, electionId, maxFactor, adnlAddr);

        TvmBuilder signatureBuilder;
        signatureBuilder.store(signature.toSlice());

        builder.storeRef(signatureBuilder);
    ...
    function _removeExpiredTransactions() inline private {
        ...
        optional(uint64, Transaction) minTransaction = m_transactions.min();
        if (!minTransaction.hasValue()) { return; }

        (uint64 trId, Transaction txn) = minTransaction.get();
        ...
        while (needCleanup && i < MAX_CLEANUP_TXNS) {
            ...
            minTransaction = m_transactions.next(trId);
            if (!minTransaction.hasValue()) { break; }

            (trId, txn) = minTransaction.get();
    ...
    function getTransactions() public view returns (Transaction[] transactions) {
        ...
        optional(uint64, Transaction) minTransaction = m_transactions.min();
        while (minTransaction.hasValue()) {
            (uint64 id, Transaction txn) = minTransaction.get();
            // returns only not expired transactions
            if (id > bound) {
                transactions.push(txn);
            }
            minTransaction = m_transactions.next(id);
        }
    }
    ...
    function getCustodians() public view returns (CustodianInfo[] custodians) {
        optional(uint256, uint8) minCustodian = m_custodians.min();
        while (minCustodian.hasValue()) {
            (uint256 key, uint8 index) = minCustodian.get();
            custodians.push(CustodianInfo(index, key));
            minCustodian = m_custodians.next(key);
        }
    }

    // @dev Get-method that returns assigned node info
    // @return node Assigned node public key
    function getNode() public view returns (uint256 pubkey) {
        pubkey = m_nodeKey;
    }
